...
class Graph():
    ...
    def double_array_size(self):
        self.vertices = self.vertices.copy() + [Station()] * len(self.vertices) * 2
    ...
    def has_edge(self, vertex1, vertex2):
        ...
        for edge in self.edges:
            if (edge.vertex_in == vertex1 and edge.vertex_out == vertex2) or (edge.vertex_out == vertex1 and edge.vertex_in == vertex2):
                return edge.weight
    ...
    def get_adjacency_matrix(self):
        ...
        """
        #Creating an NxN adjacency matrix for the graph with all infinity
        adjacency_matrix = []
        for i in range(self.get_number_of_vertices()):
            adjacency_matrix.append([0 for i in range(self.get_number_of_vertices())])

        if self.undirected_graph is False:
            for i in range(self.get_number_of_vertices()):
                adjacency_matrix[i][i] = 0
            for edge in self.get_edges():
                if adjacency_matrix[edge.vertex_out][edge.vertex_in] == math.inf:
                    adjacency_matrix[edge.vertex_out][edge.vertex_in] = 1
            ...
            for edge in self.get_edges():
                #if adjacency_matrix[edge.vertex_out][edge.vertex_in] == math.inf:
                adjacency_matrix[edge.vertex_out][edge.vertex_in] = 1
                adjacency_matrix[edge.vertex_in][edge.vertex_out] = 1
                #print(edge.weight)
            return adjacency_matrix
    ...
    def get_adjacent_vertices(self, vertex):
        ...
        """
        if vertex is None or vertex >= self.num_vertices:
            ...
            for counter, i in enumerate(adjacent_matrix[vertex]):
                if i != 0:
    ...
    def get_weighted_adjacency_matrix(self):
        ...
        adjacency_matrix = []
        for i in range(self.get_number_of_vertices()):
            adjacency_matrix.append([math.inf for i in range(self.get_number_of_vertices())])
        ...
        if self.undirected_graph is False:
            ...
            for edge in self.get_edges():
                if adjacency_matrix[edge.vertex_out][edge.vertex_in] == math.inf:
                    adjacency_matrix[edge.vertex_out][edge.vertex_in] = edge.weight
            return adjacency_matrix
            ...
            for edge in self.get_edges():
                adjacency_matrix[edge.vertex_out][edge.vertex_in] = edge.weight
                adjacency_matrix[edge.vertex_in][edge.vertex_out] = edge.weight
            return adjacency_matrix
